...
module Import
  class DummyImporter < CommonImporter
    def record_class
      Text
    end
    def parser
...
describe Import::CommonImporter do
  let(:admin_set_id) { nil }
  let(:visibility) { nil }
  let(:dir) { File.join(fixture_path, 'tei') }

  let(:importer) { Import::DummyImporter.new(dir, { visibility: visibility, admin_set_id: admin_set_id }) }
  ...
  describe 'inititialize' do
    context 'when visibility is not given' do
      subject { Import::DummyImporter.new(dir) }

      it 'sets the root directory and default visibility' do
